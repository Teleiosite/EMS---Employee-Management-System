              ...
              <tbody className="divide-y divide-gray-100">
                {filteredCandidates.map((c) => (
                  <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${c.status === 'SHORTLISTED' ? 'bg-blue-100 text-blue-800' :
                          c.status === 'REJECTED' ? 'bg-red-50 text-red-800' :
                            c.status === 'HIRED' ? 'bg-green-100 text-green-800' :
                              c.status === 'INTERVIEWING' ? 'bg-purple-100 text-purple-800' :
