...
import { Search, Filter, MoreVertical, Mail, Edit, Trash2 } from 'lucide-react';
import { employees } from '../services/mockData';

const Employees: React.FC = () => {
  const navigate = useNavigate();
  const [activeActionId, setActiveActionId] = useState<string | null>(null);
  // Use local state to reflect deletions immediately
  ...
  const handleDelete = (id: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this employee?")) {
      ...
      // Update mock data source so it persists during session
